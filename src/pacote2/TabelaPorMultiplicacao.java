...
public class TabelaPorMultiplicacao {
    private Registro[] vetor;
    private Integer tamanho;
    private Integer colisoes;

    public TabelaPorMultiplicacao(int tamanho){
    ...
    public void inserirNaTabela(Registro registro){
        int lugar = (int) (tamanho * ((pegaChave(registro) * 0.618033988749895) % 1));
    ...
    public void buscarNaTabela(int chave){
        int lugar = (int) (tamanho * ((chave * 0.618033988749895) % 1));
        for (int i = 0; i < tamanho; i++) {
            if (lugar == i){
